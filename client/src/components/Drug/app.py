...
def generate_smiles():
    ...
    # Convert name to SMILES if input is alphabetical
    ...
    print(f"[INFO] Received input: '{smiles_input}' | Requested number of outputs: {num}")

    # Use cache if available
    cache_key = (smiles_input, num)
    if cache_key in smiles_cache:
        print(f"[INFO] Using cached result for input: '{smiles_input}'")
        for res in smiles_cache[cache_key]:
            print(f"Updated SMILES Cache: {{'{smiles_input}': '{res['smiles']}'}}")
        return jsonify({"results": smiles_cache[cache_key]})

    results = []
    for _ in range(num * 2):  # Generate more and keep valid ones
    ...
    # Save to cache
    smiles_cache[cache_key] = results
...
def clear_cache():
    smiles_cache.clear()
    print("[INFO] SMILES cache cleared.")
    return jsonify({"message": "SMILES cache cleared."})
